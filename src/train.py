...
N_WAY       = 3
K_SHOT      = 10   # increased: more support = better prototypes
Q_QUERY     = 5
EPISODES    = 1500
UNFREEZE_AT = 300  # longer frozen phase with more episodes
...
optimizer = torch.optim.Adam(
    ...
    lr=1e-3, weight_decay=1e-4  # weight_decay = L2 regularisation → reduces overfitting
)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
    optimizer, T_max=EPISODES, eta_min=1e-5  # smooth cosine decay instead of step drops
)

torch.save(dataset.classes, "class_list.pth")
print(f"Classes: {dataset.classes}")
print(f"\nStarting: {N_WAY}-way {K_SHOT}-shot | {EPISODES} episodes\n")
...
for episode, (support_images, support_labels, query_images, query_labels) in enumerate(loader):
    ...
    if episode == UNFREEZE_AT:
        ...
        optimizer = torch.optim.Adam([
            {"params": model.feature.parameters(),   "lr": 5e-5, "weight_decay": 1e-4},
            {"params": model.projector.parameters(), "lr": 2e-4, "weight_decay": 1e-4},
        ])
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=EPISODES - UNFREEZE_AT, eta_min=1e-6
        )
        print(f"\nPhase 2 (ep {episode+1}): Backbone UNFROZEN — fine-tuning all layers\n")
